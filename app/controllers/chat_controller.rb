...
class ChatController < ApplicationController
    ...
    def index
        begin
            ...
            @sub_query = Conversation.select("conversations.room_id as id, count(conversations.unread) as notification").where("conversations.destination = ?", @participant).group("room_id").to_sql
            @get_conversation_rooms_data = ConversationRoom.select("conversation_rooms.id, u1.name as participant_one, u2.name as participant_two, c.message as message, unread.notification")
